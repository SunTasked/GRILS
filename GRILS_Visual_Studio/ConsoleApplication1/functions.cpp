// réalisation des fonctions de "fonctions.h"
...
void buildTurns(vector<Vertex *> & v_vertices_STW, vector<Turn *> & v_Turns, float & greed) {
	...
	while (countInsertible(v_vertices_STW, v_Turns.size()) > 0) {
		...
		for (auto it_vtx = v_vertices_STW.begin() + 2; it_vtx != v_vertices_STW.end(); it_vtx++) {
			...
			for (int m_turn = 0; m_turn < v_Turns.size(); m_turn++) {
				...
				if ((*it_vtx)->v_c_ICN[m_turn] != 'C') {
					continue; // we don't compute shifts (costly operation) if the vertex is not insertable
				}

				// for each interval in the turn where insertion is possible:
				for (int j_inter = 0; j_inter < v_Turns[m_turn]->GetIntervals(); j_inter++) {
					if (v_Turns[m_turn]->GetShift(*it_vtx, j_inter) < 0) continue;


					//-------- Computing : Time importance in tour of index m_tour. ---------//
					float impTime = 0;

					// for each vertex (considered for insertion and insertible in the interval)
					for (int i_vtx = 2; i_vtx < v_vertices_STW.size(); i_vtx++) {
						...
						impTime += f_shift_j * v_vertices_STW[i_vtx]->score;
					}
					impTime = impTime / v_Turns[m_turn]->GetAvailableTime();
					impTime = impTime * v_Turns[m_turn]->GetShift(*it_vtx, j_inter);


					//------ Computing : knapSack-constraints importance in tour of index m_tour. -----//
					float impKnapSacks = 0;
					float impknapZ = 0;

					// for each knapsack constraint:
					for (int knap = 0; knap < 10; knap++) {
						if (int((*it_vtx)->v_constraint[knap]) == 0) continue;

						impknapZ = 0;
						// for each vertex (considered for insertion)
						for (int i_vtx = 2; i_vtx < v_vertices_STW.size(); i_vtx++) {
							if (v_vertices_STW[i_vtx]->v_c_ICN[m_turn] != 'C') continue;
							impknapZ += v_vertices_STW[i_vtx]->v_constraint[knap] * v_vertices_STW[i_vtx]->score;
						}

						impknapZ = impknapZ / v_Turns[m_turn]->GetSlack(knap);
						impknapZ = impknapZ * (*it_vtx)->v_constraint[knap];

						impKnapSacks += impknapZ;
					}
					...
					for (int i_vtx = 2; i_vtx < v_vertices_STW.size(); i_vtx++) {
						if (v_vertices_STW[i_vtx]->v_c_ICN[m_turn] != 'C') continue;
						impknapZ += v_vertices_STW[i_vtx]->fee * v_vertices_STW[i_vtx]->score;
					}
					impknapZ = impknapZ / Turn::f_BudgetSlack;
					impknapZ = impknapZ * (*it_vtx)->fee;

					impKnapSacks += impknapZ;

					// heuristic value for vertex it_vtx in turn m_turn at position j_inter
					float f_Hvalue = ((*it_vtx)->score*(*it_vtx)->score) / (impKnapSacks + impTime);
